"""
@author: Diego Jimenez Puche (Modificacion del codigo de Waveshare team)
"""
...
class ADS1256:
    ...
    def ADS1256_ReadChipID(self):
        ...
        self.ADS1256_WriteReg(REG_E['REG_STATUS'],0x34)
        config.delay_ms(10)
    ...
    def ADS1256_Read_ADC_Data(self):
        ...
        if (read & 0x800000): #Comprobacion si el numero es negativo
            read &= 0x7FFFFF #Limpiar el bit del signo
            read -= 0x800000 #Obtenemos valor en negativo
        
        #Formula general --> read = ((read/0x7fffff)*(2*Vref/PGA))-Vref/PGA 
        read = ((read/0x7fffff)*0.078125)-0.0390625 #PGA=64 #Vref=2.5V
        #read = ((read/0x7fffff)*0.09375)-0.046875 #PGA=64 #Vref=3V
        return read 
    ...
    def ADS1256_GetChannalValue(self, Channel):
        if(ScanMode == 0):# 0  Single-ended input  8 channel1 Differential input  4 channe 
            if(Channel>=8):
                return 0
            self.ADS1256_SetChannal((Channel+1)%8)
            #config.delay_us(4) #t11 = 24 * 1 / 7.68 MHz = 3.125 us
            self.ADS1256_WriteCmd(CMD['CMD_SYNC'])
            #config.delay_us(4) #t11 = 24 * 1 / 7.68 MHz = 3.125 us
            # config.delay_ms(10)
            self.ADS1256_WriteCmd(CMD['CMD_WAKEUP'])
            #config.delay_us(1) #t11 = 4 * 1 / 7.68 MHz = 0.52 us
            # config.delay_ms(200)
            Value = self.ADS1256_Read_ADC_Data()
        else:
            if(Channel>=4):
                return 0
            self.ADS1256_SetDiffChannal((Channel+1)%4)
            self.ADS1256_WriteCmd(CMD['CMD_SYNC'])
            # config.delay_ms(10) 
            self.ADS1256_WriteCmd(CMD['CMD_WAKEUP'])
            # config.delay_ms(10) 
 ...
 # #===================== Pruebas de adquisicion =============================== 
 #    def ADS1256_Read_ADC_Continuos_Mode(self,Channel,numOfMeasure):
 ...
 #        #Set single/differential analog input channel
 ...
 #        config.delay_us(8)  #min delay: t6 = 50 * 1/7.68 MHz = 6.5 us
 ...
 #        for i in range(0,numOfMeasure,1):
 #            self.ADS1256_WaitDRDY()
 #            buf = config.spi_readbytes(3)
 #            read = (buf[0]<<16) | (buf[1]<<8) | (buf[2])
 #            if (read & 0x800000): #Comprobacion si el numero es negativo
 ...
 #                read -= 0x800000 #Obtenemos valor en negativo 
 #            data.append(read)
 #            #currentTime.append(time.time()-timeStart)
 #            #config.delay_us(8) 
 ...
 #    def ADS1256_Read_ADC_Continuos_Mode_time(self,Channel):
 #    # Continuously acquire analog data from one single-ended analog input.
 #    # Allows sampling of one single-ended input channel up to 30,000 SPS.
...
#    def ADS1256_Read_ADC_All_Channel(self,numOfMeasure):
#        
#        data=[]
#        
#        for i in range(0,numOfMeasure,1):
#            
#            config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
#            config.spi_writebyte([CMD['CMD_RREG'] | 0, 0x0A])
#            buf = config.spi_readbytes(24)
#            config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
#            status_byte=buf[0]
#            read = (buf[1]<<16) | (buf[2]<<8) | (buf[3])
#        
#            if (read & 0x800000): #Comprobacion si el numero es negativo
#                read &= 0x7FFFFF #Limpiar el bit del signo
#                read -= 0x800000 #Obtenemos valor en negativo
